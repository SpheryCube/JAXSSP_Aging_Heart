...
# Date: July 30, 2018 
# For each hotspot, create allele effect plots for each gene that maps to them and then create
# plots showing how the genotype's effect on the expression (either protein or sex) 
# changes with different levels of the interactive covariate.
...
##############################################
##############################################
expr_v_intcovar_by_genotype_plots <- function(expr.data, genotypes_snp, snpinfo, annot.sample, symbol, id, intcovar_name) {
  ...
  if (intcovar_name == "Age") {
    ...
    print(p)
  } else if (intcovar_name == "Sex") { 
    summary_df <- summarySE(data.w.max.snp.genos, measurevar="expr", groupvars=c("genotype", "sex"), na.rm = TRUE)
    summary_df <- na.omit(summary_df)
    p <- ggplot(summary_df, aes(x = sex,  y = expr, color = genotype, group = genotype)) +
      geom_errorbar(aes(ymin=expr-se, ymax=expr+se), width=.1, position=position_dodge(0.1)) +
      geom_line(position=position_dodge(0.1)) +
      geom_point(position=position_dodge(0.1), size=3) +
      ggtitle(label = paste0(symbol, " (", id, ") ", "Expression vs Sex \n", position_label)) +
      ylab(label = "Mean Normalized Expression Level") +
      xlab(label = "Sex") +
      labs(color = "Genotype") +
      theme_minimal() +
...
# for (hotspot_output_dir in hotspot_output_dirs) {
  # hotspot_output_dir <- "./protein_sex_hotspots_output"
  ...
  foreach (file_suffix = gene_list_files) %dopar% {
    ...
    foreach (i = 1:nrow(interesting_genes)) %dopar% {
      ...
      pdf(paste0(symbol, "_story_", id ,"_chr", qtl_chr,"_", tolower(intcovar_name), ".pdf"))
      ###################################################################
      ###################################################################
      ...
      if (intcovar_name != "None") {
        ...
        # Plot title
        max_pos <- max(scan_output_add, gmap[qtl_chr])$pos
        if (max_pos > median(gmap[qtl_chr][[1]])) {pos <- "topleft"} else {pos <- "topright"}
        title(paste0(symbol, " (", id, ") ", qtl_type))
      }
      ...
      # Make genotype plots
      expr_v_intcovar_by_genotype_plots(expr.data = expr.data,  symbol = symbol, id = id, 
                                   snpinfo = snpinfo, genotypes_snp = genotypes_snp, annot.sample = annot.sample, intcovar_name = intcovar_name) 
      ...
      if (expr_type == "protein") {
        ...
        expr_v_intcovar_by_genotype_plots(expr.data = expr.data.mrna,  symbol = tools::toTitleCase(tolower(symbol)), id = gene_id, 
                                     snpinfo = snpinfo, genotypes_snp = genotypes_snp, annot.sample = annot.sample, intcovar_name = intcovar_name) 
