...
# so that it would work with heart data set and so that it doesn't run from the command line.
...
anova_tests_2 <- function(x) {
  ...
  tmp.full <- lm(x ~ Age + Sex + Generation, data=annot.sample)
  ...
  tmp.age  <- lm(Age ~ Sex + Generation, data=annot.sample[pres,])
  tmp.sex  <- lm(Sex ~ Age + Generation, data=annot.sample[pres,])
...
anova_tests_3 <- function(x,y) {
  # full model without interaction
  tmp.full <- lm(x ~ Age + Sex + Generation + y, data=annot.sample)
  lm.full <- tidy(tmp.full)
  # the model without Age
  tmp.noage <- lm(x ~ Sex + Generation + y, data=annot.sample)
  lm.sex <- tidy(tmp.noage)
  # the model without Sex
  tmp.nosex <- lm(x ~ Age + Generation + y, data=annot.sample)
  ...
  tmp.age  <- lm(Age ~ Sex + Generation + y[pres], data=annot.sample[pres,])
  tmp.sex  <- lm(Sex ~ Age + Generation + y[pres], data=annot.sample[pres,])
  tmp.y    <- lm(y[pres] ~ Sex + Age + Generation, data=annot.sample[pres,])
  sigma.full <- sd(tmp.full$resid, na.rm = TRUE)
  sigma.age <- sd(tmp.age$resid, na.rm = TRUE)
  sigma.sex <- sd(tmp.sex$resid, na.rm = TRUE)
  sigma.y <- sd(tmp.y$resid, na.rm = TRUE)
  tmp.noage.y <- lm(y[pres] ~ Sex + Generation, data=annot.sample[pres,])
  tmp.nosex.y <- lm(y[pres] ~ Age + Generation, data=annot.sample[pres,])
...
anova_tests_int <- function(x,y) {
  ...
  lm.x <- tidy(lm(x ~ Age * Sex + Generation, data=annot.sample))
  lm.y <- tidy(lm(y ~ Age * Sex + Generation, data=annot.sample))
  lm.x.y <- tidy(lm(x ~ Age * Sex + Generation + y, data=annot.sample))
  lm.y.x <- tidy(lm(y ~ Age * Sex + Generation + x, data=annot.sample))
...
for (v in vars)
  for (u in sort(unique(annot.sample[,v]))) {
    sel <- annot.sample[,v] == u
...
## normalize everything to mean=0, sd=1
## for easy calculation of partial correlation coefficients
for (i in 1:N[["pairs"]]) {
  expr.mrna[,i] <- scale(expr.mrna[,i])
  expr.protein[,i] <- scale(expr.protein[,i])
}
annot.sample$Age <- scale(annot.sample$Age)
annot.sample$Sex <- scale(as.numeric(factor(annot.sample$Sex)))
...
# ANOVA for Protein | mRNA expression
print("Testing for dependence between protein and Age/Sex conditioned on mRNA...")
for (i in 1:N[["pairs"]]) {
  if (i %% 100 == 0) print(i)
...
# reorder columns - p-values first, correlation coefs second
...
test.age <- model.matrix(~ Age, data=annot.sample)[,-1]
covar.age<- model.matrix(~ Sex + Generation, data=annot.sample)[,-1]
test.sex <- model.matrix(~ Sex, data=annot.sample)[,-1]
covar.sex<- model.matrix(~ Age + Generation, data=annot.sample)[,-1]
...
annot.cols <- c("protein_id",	"gene_id",	"symbol",	"chr",	"start",	"end",
                "strand",	"biotype")
...
write.csv(output, file = paste0(output_prefix, "anova_output.csv"), row.names=FALSE)
